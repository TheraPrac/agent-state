...
func TestCloseActiveForced(t *testing.T) {
	...
	code := Close(s, cfg, "T-003", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", NoAC: true, Force: true})
	if code != 0 {
...
func TestCloseAlreadyClosed(t *testing.T) {
	s, cfg := setupTestEnv(t)
	code := Close(s, cfg, "T-004", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", NoAC: true, })
	if code != 1 {
...
func TestCloseGateFailure(t *testing.T) {
	...
	code := Close(s, cfg, "T-001", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", NoAC: true, })
...
func TestCloseWithGateEnforcement(t *testing.T) {
	...
	code := Close(s, cfg, "T-001", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", NoAC: true, Force: false})
	if code != 1 {
