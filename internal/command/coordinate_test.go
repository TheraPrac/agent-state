...
func TestCoordinateMultiWorker_OperationalFailureStops(t *testing.T) {
	...
	superviseFn = func(cfg *config.Config, b *coordinator.Boundary,
		...
		if itemID == "T-001" {
			return 2 // operational spawn failure
		}
		return 0
	}

	// captureStdout drains stdout so the dispatch chatter does not pollute test
	// output; the fan-out-stop announcement itself goes to stderr. The
	// load-bearing assertion is that the failure code propagates and the loop
	// returns (it does not hang — drainResults reaped the clean worker).
	var rc int
	_ = captureStdout(t, func() {
		rc = Coordinate(s, cfg, CoordinateOpts{MaxItems: 0, PollInterval: time.Millisecond})
	})
	if rc != 2 {
		t.Fatalf("operational failure must propagate its rc (2), got %d", rc)
	}
}
