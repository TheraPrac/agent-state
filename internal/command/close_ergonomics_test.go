...
func TestCloseRemovesTestLogs(t *testing.T) {
	...
	if rc := Close(s, cfg, "T-001", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", NoAC: true, }); rc != 0 {
...
func TestCloseCaseCorrectionAccepted(t *testing.T) {
	...
	var rc int
	stderr := captureStderr(t, func() int {
		rc = Close(s, cfg, "T-001", "Archived", CloseOpts{AllowMissingCapture: "test: capture gate not under test", NoAC: true, })
		return rc
	})
	if rc != 0 {
