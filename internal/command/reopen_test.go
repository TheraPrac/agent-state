...
func TestReopenClearsCompletionMarkers(t *testing.T) {
	...
	if rc := Close(s, cfg, "T-001", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", NoAC: true, Force: true, Reason: "superseded"}); rc != 0 {
...
func TestReopenWorksForIssue(t *testing.T) {
	...
	if rc := Close(s, cfg, "I-001", "done", CloseOpts{AllowMissingCapture: "test: capture gate not under test", NoAC: true, Force: true}); rc != 0 {
