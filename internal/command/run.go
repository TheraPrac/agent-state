...
func buildPlanReviewPrompt(itemID string, item *model.Item) string {
	...
	b.WriteString("META-NOTE PROHIBITION: Do NOT write meta-narrative about st-CLI behavior,\n")
	...
	b.WriteString("## Tests, ## Out-of-scope, ## Risks). If you noticed unexpected st-CLI behavior\n")
	b.WriteString("during review (deprecated paths, routing surprises, etc.), put it in REMAINING\n")
	b.WriteString("CONCERNS only — the plan body must contain implementation content exclusively.\n\n")
